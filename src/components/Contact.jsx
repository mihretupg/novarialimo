...
const CARDS = [
  ...
  { icon: MapPin,        title: 'Area',     value: 'Dallas-Fort Worth', sub: 'DFW Metroplex',   href: '', highlight: false },
...
export default function Contact() {
  ...
  return (
    <section id="contact" ref={sectionRef} className="relative py-16 sm:py-24 overflow-hidden section-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className={`text-center mb-12 sm:mb-20 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <p className="text-xs font-bold tracking-[0.22em] sm:tracking-[0.3em] uppercase text-gold-400 mb-4">Get In Touch</p>
          <p className="text-theme-muted max-w-xl mx-auto leading-relaxed">
            Reach Novaria Transportation through WhatsApp, SMS, email, or phone. Our team is available 24/7 for DFW airport transfers, executive rides, family travel, and corporate transportation questions.
          </p>
          ...
          {CARDS.map((item, i) => {
            ...
            return (
              <Card
                ...
                className={`card-luxury rounded-3xl p-5 sm:p-7 transition-all duration-700 ${item.href ? 'cursor-pointer' : ''} ${
        ...
        <div className={`relative rounded-3xl overflow-hidden transition-all duration-700 delay-500 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-8 p-5 sm:p-10 md:p-14">
            <div>
              <h3 className="playfair text-3xl sm:text-4xl font-bold text-theme mb-3">Ready for DFW Luxury Transportation?</h3>
              <p className="text-theme-muted max-w-md">Book airport transfer, executive car service, hourly chauffeur service, or corporate transportation with Novaria Transportation.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-4 flex-shrink-0">
              <a href={`sms:${PHONE_NUMBER}?&body=Hi%20Novaria%2C%20I%27d%20like%20to%20book%20a%20ride.`}
                className="btn-outline flex w-full sm:w-auto items-center justify-center gap-2 px-7 py-4 rounded-full font-semibold text-sm whitespace-nowrap">
              ...
              <button onClick={scrollToBooking}
                className="btn-primary flex w-full sm:w-auto items-center justify-center gap-2 px-7 py-4 rounded-full font-bold text-sm whitespace-nowrap">
