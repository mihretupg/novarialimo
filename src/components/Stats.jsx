...
export default function Stats() {
  ...
  return (
    <section ref={sectionRef} className="relative py-16 sm:py-24 overflow-hidden section-bg">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-16 sm:mb-24">
          {STATS.map((stat, i) => (
            ...
            >
              <p className="playfair text-3xl sm:text-4xl md:text-5xl font-bold text-gradient-gold mb-2">
                <AnimatedNumber target={stat.value} inView={inView} />
              </p>
              <p className="text-[0.68rem] sm:text-xs text-theme-subtle uppercase tracking-wide sm:tracking-widest">{stat.label}</p>
            </div>
          ))}
        </div>

        <div className={`text-center mb-14 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          <p className="text-xs font-bold tracking-[0.22em] sm:tracking-[0.3em] uppercase text-gold-400 mb-4">Testimonials</p>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {TESTIMONIALS.map((testimonial, i) => (
            <div
              ...
              className={`card-luxury rounded-3xl p-5 sm:p-7 transition-all duration-700 ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
            ...
            >
              ...
              <p className="text-sm text-theme-muted leading-relaxed mb-6 italic">"{testimonial.text}"</p>
              <div className="flex items-center gap-3 pt-5 border-t border-theme">
                <div className="w-10 h-10 rounded-full bg-gradient-to-br from-gold-500/30 to-gold-600/10 border border-gold-500/20 flex items-center justify-center flex-shrink-0">
                  <span className="text-gold-400 font-bold text-sm">{testimonial.name[0]}</span>
                </div>
                <div>
                  <p className="text-sm font-semibold text-theme">{testimonial.name}</p>
                  <p className="text-xs text-theme-subtle">{testimonial.title}</p>
                </div>
              </div>
            </div>
          ))}
